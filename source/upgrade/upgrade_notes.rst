...
* `xivo-provisioning` API URL has been updated to remove the `provd` prefix and add the API version
  number, which is `0.2`. All affected services and `wazo-provd-client` have been updated.
