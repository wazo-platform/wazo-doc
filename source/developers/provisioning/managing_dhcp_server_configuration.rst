...
The files are updated with the command ``dhcpd-update``, which is also run when updating the provisioning plugins. This commands fetches configurations files from the ``provd.xivo.fr`` server.
...
* Go in :file:`dhcp/`
* Run ``make upload`` to push your modifications to ``provd.xivo.fr``. There is no ``testing`` version of these files. Once the files are uploaded, they are available for all XiVO installations.
