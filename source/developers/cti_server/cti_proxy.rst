...
want to see the raw traffic between the client and the server::
   ...
   $ cti-proxy/ctispy skaro-new --raw
...
option::
   ...
   $ cti-proxy/ctispy skaro-new --strip timenow --strip commandid --strip replyid

If you want to see only messages matching a certain key and value, use the
``--include`` option::

   $ cti-proxy/ctispy skaro-new --include class=getlist
...
The simplest invocation is::
   ...
   $ cti-proxy/ctisave skaro-new /tmp/cti-client /tmp/cti-server

To do comparison, it's often useful to strip some fields::

   $ cti-proxy/ctisave skaro-new /tmp/cti-client /tmp/cti-server --strip timenow
